...
export const init=async(sdkAppId:string)=>{
  return await TxIm.init(sdkAppId)
}

export const login=async({userSig,userId,nickName,avatar}:{
  userSig:string,
  userId:string,
  nickName:string,
  avatar:String
...
export const sendTextMessage=async({message,userId}:{message:string,userId:string})=>{
  return await TxIm.sendTextMessage(message,userId)
}

export const sendTextGroupMessage=async({message,groupId}:{message:string,groupId:string})=>{
  return await TxIm.sendTextMessage(message,groupId)
}

export const sendCustomMessage=async({type,message,userId}:{type:string,message:string,userId:string})=>{
  return await TxIm.sendCustomMessage(type,message,userId)
}

export const sendGroupCustomMessage=async({type,message,groupId}:{type:string,message:string,groupId:string})=>{
